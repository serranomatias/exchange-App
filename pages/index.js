...
export default function Home() {
  ...
  // in case when there is no initial liquidity and after liquidity gets added it keeps track of
  ...
  // certain number of 'LP' tokens
  const [removeEther, setRemoveEther] = useState(zero);
  // removeCD is the amount of 'Crypto Dev' tokens that would be sent back to the user
  // based on a certain number of 'LP' tokens that he wants to withdraw
  const [removeCD, setRemoveCD] = useState(zero);
  // amount of LP tokens that the user wants to remove from liquidity
  const [removeLPTokens, setRemoveLPTokens] = useState("0");


  /** Variables to keep track of swap funcionality */
  ...
  // keeps track of whether 'Eth' or 'Crypto Dev'token is selected. If 'Eth' is selected
  // it means that the user wants to swap some 'Eth' for some 'Crypto Dev' tokens and vice versa
  // if 'Eth' is not selected
  const [ethSelected, setEthSelected] = useState(true);
  
  
  /** Wallet connection */
  // Create a reference to the Web3 Modal (used for connecting to Metamask) which persists as
  ...
  const getAmounts = async () => {
    try {
      ...
      // gets the amount of 'CD' tokens that are present in the reserve
  ...
  /**
   * swapTokens: Swaps 'swapAmountWei' of Eth/Crypto Dev tokens with 'tokenToBeReceivedAfterSwap'
  ...
  const _swapTokens = async () => {
    try {
      ...
      // We are here using 'eq' method from BigNumber class in 'ethers.js'
   ...
   * he wants to add to the exchange. If he is adding the liquidity after the initial
   * liquidity has already been added then we calculate the crypto dev tokens he can add,
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      // call the getTokensAfterRemove from utils folder
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    // value to get access to the underlaying object
      ...
      return (
        <div>
          ...
          <footer className={styles.footer}>
            Made by <a target="_blank" href="https://www.linkedin.com/in/serrano-matias"> | Serrano Matias |</a>
          </footer>
        </div>
      );
